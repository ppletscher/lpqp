function visualizeHistory(history, objective)
...
% plot
plot(cumsum(iter), obj);
hold on;
plot(cumsum(iter), obj_qp, 'r');
plot(cumsum(iter), obj_lp, 'g');
plot(cumsum(iter), obj_decoded, 'm');
...
hold off;
legend('objective', 'QP', 'LP', 'decoded');

% show additional objectives
if (nargin >= 2)
    for idx=1:numel(objective)
        objective(idx).value
        hline(objective(idx).value);
    end
end
